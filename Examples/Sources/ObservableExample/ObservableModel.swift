...
// This example deploys to macOS 10.15, where `@Observable` only exists through
// swift-observation-polyfill. In this fork SwiftCrossUI re-exports the polyfill
// on Apple platforms only when built with `SCUI_OBSERVATION_POLYFILL=1` (see
// the root Package.swift for why), so on macOS build this example with that set.
