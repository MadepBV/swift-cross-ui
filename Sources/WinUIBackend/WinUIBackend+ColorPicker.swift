...
extension WinUIBackend: BackendFeatures.ColorPickers {
    ...
    public func updateColorPicker(
        ...
        // Covers `isEnabled`, and the color scheme that the dialog picks up.
        environment.apply(to: colorButton)
    ...
    public func setValue(ofColorPicker colorPicker: Widget, to color: SwiftCrossUI.Color.Resolved) {
        (colorPicker as! ColorButton).setColor(to: color)
    }
}
...
final class ColorButton: WinUI.Button {
    ...
    override init() {
        ...
        self.click.addHandler { [unowned self] _, _ in
            ...
            // A content dialog is hosted outside the button's subtree, so it
            // doesn't inherit the color scheme applied to it.
            dialog.requestedTheme = self.requestedTheme
            self.dialog = dialog
    ...
    /// The color last written to the swatch.
    private var resolvedColor: SwiftCrossUI.Color.Resolved?

    @MainActor
    func setColor(to color: SwiftCrossUI.Color.Resolved) {
        // This runs on every commit and each write below is a COM crossing, so
        // skip it when the color is unchanged.
        guard color != resolvedColor else {
            return
        }
        resolvedColor = color
